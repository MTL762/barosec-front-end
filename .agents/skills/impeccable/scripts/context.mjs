...
function resolveContext(cwd = process.cwd(), options = {}) {
  ...
  // gallary in ordinary repos behave the same way.
...
function resolveCandidateContextSummary(repoRoot, projectRoot, targetPath) {
  ...
  return {
    gallarytatus: contextSourceStatus(ctx.productPath, repoRoot, projectRoot),
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    'Show each app with its gallarytatus/productPath and designStatus/designPath so the user can see child overrides, inherited root files, fallback files, or missing files before choosing. ' +
