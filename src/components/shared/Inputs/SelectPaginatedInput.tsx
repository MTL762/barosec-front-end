...
import { GetData } from "@/utils/actions/get.api";
...
Readonly<SelectPaginatedProps>) {
			...
			try {
				...
				let data = await GetData({
					...
					params: {
						...
						per_page: (currentPage * 15).toString(), // Adjust the number of items per page as needed
					},
				});
				...
				if (isNewSearch) {
					setApiOptions((prevOptions) => {
						...
						newOptions.forEach((newOption) => {
							if (
								!combined.some((existing) => existing.value === newOption.value)
							) {
	...
	const SelectComponent = allowNew ? CreatableSelect : Select;
		...
		if (!isMulti && value && !allOptions.find((opt) => opt.value === value)) {
			(async () => {
				const data = await GetData({
					endPoint: `${apiUrl}?id=${value}`,
					revalidate: 60,
					tags: [apiUrl],
				});
				console.log(data, name, "sadd2edas");
				if (data?.data?.length == 1 || data.length == 1) {
					const formattedData = Array.isArray(data?.data) ? data.data : data;
					setApiOptions((prev) => [
						...prev,
						{
							label: formattedData[0][labelKey] || "Unknown",
	...
	return (
		<div className="flex flex-col w-full gap-3">
			<SelectComponent
				...
				value={
					selectedValue as unknown as SingleValue<Option> | MultiValue<Option>
				}
